...
def main():
    ...
    p.add_argument('--exception-date');p.add_argument('--exception-reason')
    ...
    if a.command=='bootstrap':result={'baseline_verified':bool(bootstrap())}
        ...
        from .preopen import run_official
        result=run_official(s,a.exception_date,a.exception_reason)
    elif a.command=='event':result=run_daily(s,a.command,a.event_id)
        ...
        from .preopen import replay_any
        result=replay_any(s,a.run_id)
